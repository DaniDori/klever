...
По HTTP сертификат не нужен, поэтому чужое имя туда всё же доходит. Certbot
оставляет в блоке на 80-м порту `return 404` — замените его на переезд к себе:

```nginx
    return 301 https://klever.ru$request_uri;
```

Для поисковика это внятнее, чем 404: 301 говорит «это тот же сайт, вот его
настоящий адрес», и дубль склеивается с оригиналом, а не ждёт, пока протухнет.
Свои домены до этой строки не доходят — они уходят на https по своему же имени
правилами выше.

Проверить можно так — свой домен отвечает 200, чужой по HTTPS не отвечает
вовсе, а по HTTP приводит к вам:
