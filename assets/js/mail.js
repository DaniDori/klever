...
window.Mail = (function () {
  ...
  /* Сколько ждём ответа сервиса — на одну попытку. Попыток две, поэтому
     покупатель в худшем случае стоит перед кнопкой примерно те же секунды,
     что и раньше при одной длинной, зато с двумя заходами вместо одного. */
  var TIMEOUT = 8000;
  ...
  /* Сервис молчит дольше отведённого — запрос оборвал наш собственный таймер */
  ...
  function post(url, body, retried) {
    ...
    return fetch(url, {
      ...
      /* Мобильный интернет часто рвётся на первом запросе, а второй раз
         кнопку никто не нажимает. Повторяем и обрыв связи, и молчание
         сервиса — но только пока ответа не было: если он пришёл, повтор
         означал бы второе такое же письмо.

         На молчании письмо всё же могло уйти, и тогда повтор принесёт
         второе. Это осознанный размен: получить письмо о заказе дважды
         неприятно, не получить вовсе — хуже. */
      if (!retried && !responded && (isNetworkError(e) || isTimeout(e))) {
  ...
  function explain(raw) {
    ...
    if (m.indexOf('timeout') > -1 || m.indexOf('abort') > -1) {
      return 'Сервис не ответил ни с первого раза, ни со второго. ' +
        'Обычно это ненадолго — попробуйте через минуту.';
    }
